<div align="center">
  <img width="128" alt="Luma 应用图标" src="./src-tauri/icons/icon.png">
  <h1>Luma</h1>
  <p>基于Rust、Tauri 2、React 的桌面应用启动器，支持 macOS、Windows 和 Linux。</p>

  <p>
    <a href="https://github.com/ccwxl/Luma/releases/latest"><img alt="下载最新版" src="https://img.shields.io/badge/下载-最新版-2ea44f?style=for-the-badge&amp;logo=github"></a>
  </p>

  <p>
    <a href="https://github.com/ccwxl/Luma/tree/main/src-tauri"><img alt="Rust" src="https://img.shields.io/badge/Rust-000000?style=flat-square&amp;logo=rust&amp;logoColor=white"></a>
    <a href="https://github.com/ccwxl/Luma/releases/latest"><img alt="Latest release" src="https://img.shields.io/github/v/release/ccwxl/Luma?display_name=tag&amp;sort=semver&amp;style=flat-square&amp;label=release&amp;color=5c6ac4"></a>
    <a href="https://github.com/ccwxl/Luma/actions/workflows/release.yml"><img alt="发布构建状态" src="https://github.com/ccwxl/Luma/actions/workflows/release.yml/badge.svg"></a>
    <a href="https://github.com/ccwxl/Luma/releases"><img alt="Total downloads" src="https://img.shields.io/github/downloads/ccwxl/Luma/total?style=flat-square&amp;label=downloads&amp;color=blue"></a>
    <img alt="支持平台" src="https://img.shields.io/badge/平台-macOS%20%7C%20Windows%20%7C%20Linux-555?style=flat-square">
  </p>
</div>

---

## 应用预览

<div align="center">
  <img width="100%" alt="Luma 应用启动器主界面，展示搜索框和已安装应用网格" src="./docs/luma-screenshot.jpg">
  <p><sub>自动扫描本机应用，支持即时搜索、排序、分页浏览和一键启动。</sub></p>
</div>

## 界面与快捷键

界面采用模糊背景、居中搜索和应用图标网格，保留底部分页，不显示 Dock 或页内品牌文字。
页面通过 GPU `translate3d` 横向滑动；触控板和触摸滑动可跟随手势，鼠标滚轮也可翻页。

| 快捷键 | 操作 |
| --- | --- |
| `←` / `→` | 上一页 / 下一页 |
| 搜索结果中 `←` / `→` / `↑` / `↓` | 在待选应用间移动焦点 |
| `Enter` | 打开第一个搜索结果，输入法确认文字时不触发 |
| `⌘ ,`（Windows/Linux 也支持 `Ctrl ,`） | 打开设置 |
| `Esc` | 关闭 Luma（macOS 启用触发角时改为后台隐藏） |
| `⌘ F` / `Ctrl F` | 聚焦搜索 |

搜索支持按顺序字符进行模糊匹配（例如用 `vc` 找到 `VSCode`）。搜索框左侧可按安装时间或首字母排序；
右键应用可在 Finder 或当前平台的文件管理器中显示其入口。

设置中可调整背景模糊、减少动态效果以及清理图标缓存。macOS 还可选择任意屏幕触发角；
鼠标在角落停留达到设定延时后即可显示 Luma，延时可在 50–1000ms 之间调整（默认 80ms）。
启用后 Luma 关闭时会隐藏到后台，登录系统后需先启动一次。
设置保存在本机。

图标使用前端内存缓存、图片预解码和相邻页预加载；后端按批读取，并把图标缓存在应用缓存目录的
`icons/` 中。应用入口或本地图标文件更新后会自动失效。macOS 和 Windows 通过系统图标接口生成 WebView 可显示的 PNG；
Windows 由 Shell 读取快捷方式的自定义图标和程序嵌入图标，保留透明度，最高生成 256 × 256 像素。
没有可用图标的平台入口显示通用应用图标。

## 平台接口

`src-tauri/src/app.rs` 是应用入口，负责 Tauri 命令和应用初始化。应用管理能力由
`src-tauri/src/platform/mod.rs` 提供统一接口，使用 `cfg(target_os)` 在编译时选择实现。

| 平台 | 应用来源 | 启动方式 |
| --- | --- | --- |
| macOS | `/Applications`、`/System/Applications`、`~/Applications` 中的 `.app`，最多下探两层子目录 | `/usr/bin/open`，检查退出状态 |
| Windows | 当前用户和所有用户的开始菜单 Programs 目录，递归扫描 `.lnk`、`.exe`、`.appref-ms` | 原生 `ShellExecuteExW`，保留快捷方式参数和工作目录 |
| Linux | GIO 按 `XDG_DATA_HOME`、`XDG_DATA_DIRS` 枚举已注册的 `.desktop` | GIO `DesktopAppInfo::launch`，处理 `Exec`、`Terminal`、工作目录和 D-Bus 激活 |

前端 API：

- `invoke<AppInfo[]>("get_installed_apps")`：返回按名称排序、按路径去重的应用列表。
- `invoke("launch_app", { appPath: app.path })`：启动所选应用；失败时 Promise reject。
- `invoke("get_app_icons", { appPaths })`：批量返回图标 data URL，只接受当前扫描列表中的应用，单批最多 12 个。
- `invoke("clear_icon_cache")`：清理 Luma 自身生成的图标缓存。
- `invoke("close_app")`：退出应用。
- `AppInfo` 字段为 `name: string`、`path: string`、`icon_path: string | null`、`installed_at: number | null`。

`path` 是平台启动入口的绝对路径，前端应原样传回。`installed_at` 优先使用入口创建时间，并在平台不支持时回退到修改时间；
`icon_path` 仅在已取得本地图标文件时返回；
Windows 的 `icon_path` 返回 `null`，图标通过 `get_app_icons` 提取；Linux 主题图标目前返回 `null`。Windows 列表覆盖开始菜单中注册的应用，
没有开始菜单入口的便携应用或商店应用不在扫描范围内。Linux 遵循桌面环境的隐藏规则及用户级覆盖规则。

扫描和启动运行在 Tauri 后台线程。新增平台能力时，先扩展 `ApplicationApi`，再补齐各平台实现。

## 开发与验证

先安装 [Tauri 对应系统依赖](https://v2.tauri.app/start/prerequisites/)、Rust、Node.js 22.12+ 和 pnpm。

```sh
pnpm install
pnpm tauri dev
```

```sh
pnpm build
cargo fmt --manifest-path src-tauri/Cargo.toml -- --check
cargo test --manifest-path src-tauri/Cargo.toml --lib --locked
cargo clippy --manifest-path src-tauri/Cargo.toml --all-targets --locked -- -D warnings
```

需要在浏览器中预览本机应用时，先运行 `cargo run --manifest-path src-tauri/Cargo.toml --example export_preview`，
再运行 `pnpm dev`。生成的 `.preview/` 只保存在本机，不进入版本控制或生产包；浏览器预览不会启动本机应用。

图标缓存性能检查：`cargo run --manifest-path src-tauri/Cargo.toml --example benchmark_icons`。
该检查分别测量首次生成、内存读取和新缓存实例的磁盘读取，不包含前端渲染或 IPC 耗时。

GitHub Actions 在三个系统上分别构建前端、编译 Rust 并运行对应平台的测试。
测试使用临时目录，覆盖扫描、过滤、去重和无效启动入口，不会打开真实应用。
实际桌面环境中的应用启动仍需在对应系统进行验证。

## 更新日志

### v0.1.3
- 🖱️ **macOS 触发角**：可在设置中选择任意屏幕角，停留鼠标即可唤起 Luma；关闭后保持后台运行。
- ⏱️ **触发延时可调**：可在 50–1000ms 之间调整触发延时，步进 10ms，默认 80ms，修改后立即生效。
- 🖥️ **前台唤醒与多屏稳定性**：即使其他应用处于前台，Luma 也能正常激活；支持动态刷新显示器布局。
- 🔁 **后台重新打开**：Luma 隐藏后再次点击应用图标可恢复窗口，唤醒后自动聚焦搜索框。

### v0.1.2
- 🔎 **模糊搜索**：支持按顺序字符匹配应用名称，例如输入 `vc` 即可找到 `VSCode`。
- ↕️ **灵活排序**：可按安装时间或应用名称升降序排列，更快找到最近安装的应用。
- ⌨️ **键盘导航增强**：搜索结果支持方向键在应用网格中移动焦点，并可跨页连续导航。
- 📂 **在文件管理器中定位**：右键应用即可在 Finder、文件资源管理器或 Linux 文件管理器中显示其入口。

### v0.1.1
- 🪟 **沉浸式无边框全屏**：窗口默认去除标题栏和红黄绿按钮，启动时自动贴合屏幕可用区域，保留顶部系统菜单栏。
- 🧊 **macOS 原生实时毛玻璃**：采用系统级 `FullScreenUI` Vibrancy 材质，桌面壁纸和后台窗口自然透光磨砂模糊；Windows 支持 Mica 磨砂兼容。
- 🔘 **20px 原生精美圆角**：窗口与应用主界面四个角统一为 20px 圆角裁切，配备 1px 微透精致边框。
- ⚡ **闪电快捷退出体系**：
  - 支持键盘 `Esc` 退出（优先关闭设置/清空搜索，次级退出）。
  - 支持鼠标点击图标外的任意空白区域直接退出。
  - 点击并启动目标应用后自动退出，无缝切换到打开的应用。
- 📱 **7 × 5 大网格布局优先**：全屏及大屏环境下优先展现 7 列 × 5 行（单页 35 个应用），排版更充实、呼吸感更好。
- 🖼️ **深色应用图标与 Favicon 升级**：全新设计的高清深色应用图标及浏览器标签图标。
- ⚙️ **设置面板与图标缓存机制**：支持调节背景模糊度、减少动态效果，并加入两级图标持久化缓存与手动清理功能。
- 🪟 **Windows 平台图标增强**：完善 Windows 平台应用图标动态提取与透明通道渲染。

### v0.1.0
- 🎉 **首个版本发布**：基于 Tauri 2 + React + TypeScript 构建的现代化桌面应用启动器。
- 🖥️ **跨平台支持**：全面支持 macOS、Windows 与 Linux 桌面平台。
- 🔍 **应用智能扫描**：自动扫描系统已安装应用，支持实时搜索与过滤。
- 🚀 **一键快速启动**：点击应用图标即可直接调起本地对应桌面应用程序。
- 🎨 **Launchpad 原生美学**：极简毛玻璃界面、自适应网格布局与流畅的多页手势/键盘滑动切换。
